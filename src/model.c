...
void	model_make_iso(t_model *m)
{
	...
	m->mat_obj = mat4s_mult(mat_rot(0, angle), mat_rot(2, M_PI / 4));
	m->mat_cam = mat4s_mult(mat_trans(1, WIN_HEIGHT / 2.f), mat_trans(0, WIN_WIDTH / 2.f));
	m->mat_cam = mat4s_mult(mat_trans(2, -2*m->hmax),m->mat_cam);
}
