...
static void	model_init_mat(t_model *model)
{
	...
	model->mat_obj = mat4s_mult(mat_trans(2, 2 * model->hmax - model->hmin), model->mat_obj);
...
*/
void	model_perspective_vertex(t_model *model, t_vec4 *v)
{
			v->e[0] = v->e[0] * (model->hmin - model->hmax) / v->e[2];
			v->e[1] = v->e[1] * (model->hmin - model->hmax) / v->e[2];
}
